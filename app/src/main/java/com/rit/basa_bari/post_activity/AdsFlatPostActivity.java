...
public class AdsFlatPostActivity extends AppCompatActivity  implements View.OnClickListener{
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.submitBTN) {
            if(mUploadTask!=null){
                Toast.makeText( AdsFlatPostActivity.this, "Image is progess", Toast.LENGTH_SHORT).show();
            }else {
        ...
        if (imageUri != null) {
            ...
            mUploadTask=  storageReference.putFile( imageUri ).addOnSuccessListener( new OnSuccessListener<UploadTask.TaskSnapshot>() {
                ...
                public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                    ...
                    String flatMonth=month.getSelectedItem().toString().trim();
                    String flatGender=gender.getSelectedItem().toString().trim();
                    Integer flatRent= Integer.valueOf( rent.getText().toString().trim() );
                    String flatPhone=phone.getText().toString().trim();
                    String flatLocation=location.getText().toString().trim();
                    String flatDescription=description.getText().toString().trim();

                    MessUpload messUpload=new MessUpload( mStorageReference.getDownloadUrl().toString(), flatMonth, flatGender,
                                                          flatRent, flatPhone, flatLocation, flatDescription);
